...
class Lexer(private val expression: String) {
    ...
    private val isFirstMinus = expression[0] == '-'
    private val localExpr = if (isFirstMinus) "0$expression" else expression
    val tokens : List<Token> = run{
        val matchedTokens = Regex(Token.RegexStr.allTokens).findAll(localExpr)
        var t = matchedTokens.map { Token(it.value) }.toMutableList()
        if (isFirstMinus) t.drop(0)
        t.toList()
    }
    // lazy evaluation
    ...
    fun isCorrect() : Boolean {
        ...
        if(localIsCorrectVar == null) {
            val matchedTokensAndSpaces = Regex(regexCheckCorrectness).findAll(localExpr)
